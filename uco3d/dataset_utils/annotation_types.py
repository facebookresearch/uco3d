...
@dataclass
class UCO3DViewpointAnnotation:
    # In right-multiply (PyTorch3D) format. X_cam = X_world @ R + T
    R: Tuple[TF3, TF3, TF3]
    T: TF3
...
class FrameAnnotation:
    ...
    sequence_name: str
    # 0-based number of a frame in a sequence
    frame_number: int
    # timestamp in seconds from the video start
    frame_timestamp: Optional[float] = None

    image: Optional[ImageAnnotation] = None
...
class SequenceAnnotation:
    ...
    gaussian_splats: Optional[GaussianSplatsAnnotation] = None
    reconstruction_quality: Optional[ReconstructionQualityAnnotation] = None
